...
async function submitHyakumasuAnswers() {
    ...
    cells.forEach(
        (
            ...
            if (
                ...
                cell.setAttribute(
                    "aria-label",
                    `${hyakumasuState.rowNumbers[rowIndex]}たす${hyakumasuState.columnNumbers[columnIndex]}。入力${userAnswer}、正解${correctAnswer}`
                );


                cell.title =
                    `入力: ${userAnswer} / 正解: ${correctAnswer}`;
    ...
    if (incorrectAnswers.length === 0) {
        ...
        resultLines.push(
            "",
            `まちがい ${incorrectAnswers.length}問`
        );
        ...
        incorrectAnswers.forEach(
            (item) => {
                ...
                resultLines.push(
                    `${item.rowNumber}＋${item.columnNumber}：入力 ${item.userAnswer} → 正解 ${item.correctAnswer}`
                );
