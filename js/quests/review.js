...
function installReviewStyles() {
  ...
  .review-head{text-align:center;margin-bottom:16px}.review-head h2{margin:4px 0;font-size:clamp(24px,4vw,38px)}.review-sub{margin:0;color:#6a5846}.review-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
...
async function openReviewTraining() {
  installReviewStyles();
...
function bindSubjectEvents(){document.querySelector("[data-review-subject='math']")?.addEventListener("click",showMathUnits); document.querySelector("[data-review-board]")?.addEventListener("click",()=>typeof changeScreen==="function"&&changeScreen("questboard"));}
function showMathUnits(){const c=document.getElementById("questContainer"); c.innerHTML=`<div class="review-wrap"><section class="review-panel"><header class="review-head"><p class="review-badge">算数</p><h2>単元を選ぶ</h2><p class="review-sub">塾で学んだところも、未来への投資も、自由に選べます。</p></header><div class="review-grid"><article class="review-card available" data-review-unit="unit_average"><h3>単位量・平均</h3><p>平均、1あたりの量、人口密度など</p><span class="review-badge">${historyText("unit_average")}</span></article></div><button class="review-back" data-review-subjects>教科選択へ戻る</button></section></div>`; document.querySelector("[data-review-unit]")?.addEventListener("click",()=>showReviewLevels("unit_average")); document.querySelector("[data-review-subjects]")?.addEventListener("click",openReviewTraining);}
function showReviewLevels(unitId){const unit=REVIEW_QUESTIONS[unitId], daily=getReviewDaily(); const c=document.getElementById("questContainer"); c.innerHTML=`<div class="review-wrap"><section class="review-panel"><header class="review-head"><p class="review-badge">算数</p><h2>${unit.title}</h2><p class="review-sub">どの難易度からでも、何度でも挑戦できます。</p></header><div class="review-levels">${Object.entries(REVIEW_LEVELS).map(([id,l])=>`<article class="review-level" data-review-level="${id}"><h3>${l.label}</h3><p>5問・正確性×時間で加点</p><span class="review-badge ${daily.bonuses[id]?"done":""}">${daily.bonuses[id]?"本日ボーナス獲得済み":"本日初回 +2GP"}</span></article>`).join("")}</div><button class="review-back" data-review-units>単元選択へ戻る</button></section></div>`; document.querySelectorAll("[data-review-level]").forEach(el=>el.addEventListener("click",()=>startReviewQuiz(unitId,el.dataset.reviewLevel))); document.querySelector("[data-review-units]")?.addEventListener("click",showMathUnits);}
function shuffledFive(items){return [...items].sort(()=>Math.random()-.5).slice(0,5);}
function startReviewQuiz(unitId,level){reviewState={unitId,level,questions:shuffledFive(REVIEW_QUESTIONS[unitId].questions[level]),index:0,answers:[],input:"",startedAt:Date.now(),context:null}; if(!window.QuestEngine?.start)return false; return window.QuestEngine.start(`review-${level}`);}
