...
async function finishReviewQuiz(){
  ...
  try{
    if(!s.context || typeof s.context.complete!=="function"){
      throw new Error("クエスト完了処理が見つかりません。");
    }

    /*
      待機画面へ書き換えず、そのままQuestEngineの結果画面へ進む。
      changeScreenが別の遷移と重なって結果画面を開けなかった場合も、
      complete() 後に結果画面を即時表示して取りこぼさない。
    */
    await s.context.complete({
      isPerfect:correct===5,
    ...
    if(c){
      c.innerHTML=`<div class="review-wrap"><section class="review-panel review-feedback is-wrong"><h2>結果画面を開けませんでした</h2><p>修行記録は保存されています。クエストボードへ戻ってください。</p><div class="review-actions"><button type="button" class="primary" data-review-fallback-back>クエストボードへ戻る</button></div></section></div>`;
      c.querySelector("[data-review-fallback-back]")?.addEventListener("click",()=>{
